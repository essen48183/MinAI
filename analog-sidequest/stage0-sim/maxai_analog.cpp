...
// ============================================================================
//  ANALOG SIMULATION HOOKS  (Stage 0, Gate 1)
// ============================================================================
//  This file is a COPY of maxai.cpp from the book, modified to route its
//  matmuls and weight updates through a simulated Q8.8.8 analog tile with
//  parameterized noise. The book's maxai.cpp is untouched.
//
//  The hooks below are active only when --analog_noise=<sigma> is set at
//  the command line. At sigma=0 the code is numerically identical to the
//  stock MaxAI training run, which means clean-float32 baseline and
//  analog-simulated run can be directly compared on the same seed.
//
//  Two injection points:
//
//    analog_matmul_noise(K) -- returns Gaussian noise to add to each output
//        of a matmul that accumulated K terms. Noise std is sqrt(K)*sigma;
//        a small calibration residual is added at the summing node. This
//        models the "what each MAC contributes to the output" noise that
//        a real tile exhibits.
//
//    analog_weight_update(w, delta) -- replaces w += delta. Applies the
//        update through Q8.8.8 stochastic rounding: exact down to the
//        finest slice step, with probabilistic commit on the fractional
//        part. Matches the behaviour of Tile::write_delta() in
//        analog_sim.cpp so training dynamics correspond to what real
//        hardware would produce.
//
//  Wiring is mechanical: every forward matmul in forward_block() and
//  forward() gets an analog_matmul_noise() term added to its output; every
//  weight subtraction in sgd_step() becomes an analog_weight_update()
//  call. All other MaxAI code is verbatim from the book.
// ----------------------------------------------------------------------------

static float    g_analog_sigma     = 0.0f;      // per-MAC noise (fraction of full scale)
static float    g_analog_cal_resid = 0.0f;      // per-output calibration residual
static uint32_t g_sim_rng          = 0xFACEFEEDu;

static inline uint32_t analog_xs() {
    uint32_t x = g_sim_rng;
    x ^= x << 13; x ^= x >> 17; x ^= x << 5;
    return g_sim_rng = x;
}
static inline float analog_u01() {
    return (analog_xs() & 0x00FFFFFFu) / (float)0x01000000;
}
static inline float analog_gauss() {
    float u1 = analog_u01();
    float u2 = analog_u01();
...
// Noise on a matmul output. K = number of MACs accumulated in the inner
// product. Returns 0 (no-op) when analog_sigma is zero, so the code path
// is a free add in clean-math mode.
static inline float analog_matmul_noise(int K) {
    if (g_analog_sigma <= 0.0f) return 0.0f;
    return std::sqrt((float)K) * g_analog_sigma  * analog_gauss()
         +                       g_analog_cal_resid * analog_gauss();
}

// SGD weight update through Q8.8.8 stochastic rounding. Finest writable
// step is 1 / (127.5 * 65536) ≈ 1.2e-7, matching the slice-C granularity
// of the Tile class.
//
// Takes `lr` and `g` separately rather than a pre-computed `delta`. This
// is a deliberate interface choice: at sigma=0 the fast path compiles to
// `w -= lr * g`, which on arm64 becomes a single fused-multiply-subtract
// with one rounding — byte-identical to stock MaxAI. If we passed a
// pre-negated delta from outside, the compiler would have to do two
// separate roundings (multiply-then-negate, then add), and the
// accumulated bit-level drift over thousands of SGD steps shows up as
// outright training divergence at --blocks=2 --layernorm=1. Subtle
// and real. Preserve FMA fusion.
static inline void analog_weight_sub(float& w, float lr, float g) {
    if (g_analog_sigma <= 0.0f) {
        w -= lr * g;                               // FMA-fused on arm64; bit-identical to stock
        return;
    }
    const float delta = -lr * g;
    static const float STEP = 1.0f / (127.5f * 65536.0f);
    float taps = delta / STEP;
    int   int_part = (int)std::trunc(taps);
    float frac     = taps - (float)int_part;
    int   extra    = 0;
    float r = analog_u01();
    if (frac > 0.0f && r <  frac) extra =  1;
    if (frac < 0.0f && r < -frac) extra = -1;
    w += (int_part + extra) * STEP;
    // Clip to representable range [-1, 1].
    if (w >  1.0f) w =  1.0f;
    if (w < -1.0f) w = -1.0f;
}
...
static void parse_args(int argc, char** argv) {
    for (int i = 1; i < argc; ++i) {
        ...
        else if (std::strncmp(a, "--analog_noise=", 15) == 0) {
            // Stage-0 Gate-1 injection. sigma is per-MAC weight-equivalent
            // noise as a fraction of full scale. 0 = clean MaxAI; 0.003 =
            // precision analog; 0.0003 = standard analog; 0.003 = hobbyist.
            g_analog_sigma     = (float)std::atof(a + 15);
            g_analog_cal_resid = 0.1f * g_analog_sigma;   // 10% of MAC noise at the summing node
        }
...
static void forward_block(int b) {
    ...
    for (int t = 0; t < T; ++t) {
        for (int j = 0; j < D_MODEL; ++j) {
            ...
            // [ANALOG] per-matmul noise injection at each tile output
            q += analog_matmul_noise(D_MODEL);
            k += analog_matmul_noise(D_MODEL);
            v += analog_matmul_noise(D_MODEL);
    ...
    for (int t = 0; t < T; ++t)
        for (int u = 0; u < T; ++u) {
            ...
            s += analog_matmul_noise(D_MODEL);   // [ANALOG]
    ...
    for (int t = 0; t < T; ++t)
        for (int d = 0; d < D_MODEL; ++d) {
            ...
            s += analog_matmul_noise(T);   // [ANALOG]
            attn_o[b][t][d] = s;
        }
    ...
    if (cfg.use_ffn) {
        ...
        for (int t = 0; t < T; ++t)
            for (int d = 0; d < D_MODEL; ++d) {
                ...
                s += analog_matmul_noise(D_FF);   // [ANALOG]
                ff_out[b][t][d] = s;
...
void sgd_step() {
    // [ANALOG] Every weight update goes through analog_weight_sub(), which
    // at sigma=0 reduces to `w -= lr * g` (FMA-fused, byte-identical to
    // stock MaxAI) and at sigma>0 applies Q8.8.8 stochastic rounding.
    // Gradient accumulation across the batch is already digital (the
    // /batch division was applied in backward()), so we simply commit
    // whatever the gradient buffer currently holds as this step's update.
    for (int v = 0; v < VOCAB;         ++v) for (int d = 0; d < D_MODEL; ++d) analog_weight_sub(token_emb[v][d], LR_EMB, g_token_emb[v][d]);
    for (int t = 0; t < cfg.seq_len;   ++t) for (int d = 0; d < D_MODEL; ++d) analog_weight_sub(pos_emb  [t][d], LR_EMB, g_pos_emb  [t][d]);

    for (int b = 0; b < cfg.num_blocks; ++b) {
        for (int i = 0; i < D_MODEL; ++i)
            for (int j = 0; j < D_MODEL; ++j) {
                analog_weight_sub(Wq[b][i][j], LR_ATTN, g_Wq[b][i][j]);
                analog_weight_sub(Wk[b][i][j], LR_ATTN, g_Wk[b][i][j]);
                analog_weight_sub(Wv[b][i][j], LR_ATTN, g_Wv[b][i][j]);
            }
        if (cfg.use_ffn) {
            for (int i = 0; i < D_MODEL; ++i) for (int j = 0; j < D_FF;    ++j) analog_weight_sub(W1[b][i][j], LR_FFN, g_W1[b][i][j]);
            for (int i = 0; i < D_FF;    ++i) for (int j = 0; j < D_MODEL; ++j) analog_weight_sub(W2[b][i][j], LR_FFN, g_W2[b][i][j]);
        }
        if (cfg.use_layernorm) {
            // LN params use the same LR as the sub-layer they precede.
            for (int d = 0; d < D_MODEL; ++d) {
                analog_weight_sub(gain1[b][d], LR_ATTN, g_gain1[b][d]);
                analog_weight_sub(bias1[b][d], LR_ATTN, g_bias1[b][d]);
                analog_weight_sub(gain2[b][d], LR_FFN,  g_gain2[b][d]);
                analog_weight_sub(bias2[b][d], LR_FFN,  g_bias2[b][d]);
            }
        }
    }

    for (int d = 0; d < D_MODEL; ++d) for (int v = 0; v < VOCAB; ++v) analog_weight_sub(Wout[d][v], LR_OUT, g_Wout[d][v]);
    if (cfg.use_layernorm) {
        for (int d = 0; d < D_MODEL; ++d) {
            analog_weight_sub(gain_f[d], LR_OUT, g_gain_f[d]);
            analog_weight_sub(bias_f[d], LR_OUT, g_bias_f[d]);
        }
    }
}
